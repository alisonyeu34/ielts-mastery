...
interface AcademicInversionStudioProps {
...
export function AcademicInversionStudio({
  exercise,
  exerciseIndex,
  totalExercises,
  userAnswer,
    ...
    >
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-secondary/30 border border-border space-y-2">
        <span className="text-[10px] font-mono text-muted-foreground uppercase font-bold tracking-wider block">
          Câu Gốc (Thẳng Thắn / Thiếu Điểm Nhấn Tu Từ):
        </span>
      ...
      {/* Inversion Trigger Badge */}
      {exercise.triggerPhrase && (
        <div className="p-3.5 rounded-2xl bg-purple-500/[0.04] border border-purple-500/20 flex items-center justify-between gap-2 text-xs">
          <div className="flex items-center gap-2">
            <span className="font-bold text-purple-600 dark:text-purple-400">
              Cụm từ kích hoạt đảo ngữ (Trigger):
            </span>
            <span className="font-mono font-black text-foreground px-2 py-0.5 rounded-md bg-purple-500/10 border border-purple-500/20">
              "{exercise.triggerPhrase}"
            </span>
          </div>
          <span className="text-[10px] font-mono text-muted-foreground hidden sm:inline">
            Cấu trúc: Trigger + Trợ động từ + Chủ ngữ + Động từ
          </span>
        </div>
      )}
      ...
      <div className="space-y-2">
        <label className="text-xs font-bold text-foreground flex items-center justify-between">
          <span>Viết lại câu bằng cấu trúc Đảo ngữ học thuật:</span>
          <span className="text-[10px] font-mono text-muted-foreground">
            Lưu ý: Đảo trợ động từ lên trước chủ ngữ!
          </span>
        </label>
        <textarea
          rows={3}
          ...
          placeholder={`Bắt đầu bằng "${exercise.triggerPhrase || 'Not only'}..."`}
      ...
      {result && !result.hasInversionBug && (
        ...
        >
          <div className="flex items-center gap-2 font-bold">
            ...
            <span>{result.isCorrect ? "Đảo Ngữ Chuẩn Xác!" : "Chưa Hoàn Hảo:"}</span>
          </div>
      ...
      <div className="pt-2 border-t border-border/70 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="px-6 py-3 rounded-2xl bg-purple-600 hover:bg-purple-700 text-white font-black text-xs sm:text-sm shadow-md transition-all hover:scale-105 flex items-center gap-2 cursor-pointer"
        >
          <Send className="h-4 w-4" />
          <span>Kiểm Tra Đảo Ngữ</span>
        </button>
      </div>
    </div>
  );
