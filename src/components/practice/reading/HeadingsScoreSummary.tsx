...
import { MatchingHeadingsSummary } from "@/hooks/useMatchingHeadings";
import { cn } from "@/lib/utils";

interface HeadingsScoreSummaryProps {
  summary: MatchingHeadingsSummary;
  passageTitle: string;
  onRestart: () => void;
  className?: string;
}

export function HeadingsScoreSummary({
  summary,
  passageTitle,
  onRestart,
  className,
}: HeadingsScoreSummaryProps) {
    ...
    >
      ...
      <div className="space-y-1.5">
        <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
          Tổng Kết Matching Headings
        </span>
        <h3 className="text-xl sm:text-2xl font-extrabold text-foreground">
          {isMastered ? "Làm Chủ Tiêu Đề Xuất Sắc!" : "Đã Hoàn Thành Bài Ghép Tiêu Đề"}
        </h3>
      ...
      {summary.score < summary.total && (
        <div className="p-4 rounded-2xl bg-rose-500/[0.04] border border-rose-500/20 text-left space-y-2 text-xs">
          <div className="flex items-center gap-1.5 font-bold text-rose-600 dark:text-rose-400">
            ...
            <span>Đã tự động gom {summary.total - summary.score} đoạn ghép sai vào Ngân Hàng Lỗi Sai</span>
          </div>
          <p className="text-muted-foreground leading-relaxed text-[11px]">
            Hệ thống đã phân loại lỗi sai theo bẫy (Trùng khớp mặt chữ Word-match, Quá hẹp Too Narrow, hoặc Quá rộng Too Broad) để bạn luyện tập lại ở Module 5.
          </p>
        </div>
      )}
