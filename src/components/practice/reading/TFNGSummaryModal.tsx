...
import { TFNGResultSummary } from "@/hooks/useTFNGLogic";
import { cn } from "@/lib/utils";

interface TFNGSummaryModalProps {
  summary: TFNGResultSummary;
  passageTitle: string;
  onRestart: () => void;
  className?: string;
}

export function TFNGSummaryModal({
  summary,
  passageTitle,
  onRestart,
  className,
}: TFNGSummaryModalProps) {
  const isMastered = summary.accuracyPercentage >= 75;
    ...
    >
      ...
      <div className="space-y-1.5">
        <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-500/20 uppercase tracking-wider">
          Tổng Kết Bài Luyện TFNG
        </span>
        <h3 className="text-xl sm:text-2xl font-extrabold text-foreground">
          {isMastered ? "Làm Chủ Dạng Bài Xuất Sắc!" : "Đã Hoàn Thành Phiên Luyện Tập"}
        </h3>
        <p className="text-xs sm:text-sm text-muted-foreground">
          Bài đọc: <strong>{passageTitle}</strong>
        </p>
      </div>
      ...
      {summary.correctCount < summary.total && (
        <div className="p-4 rounded-2xl bg-rose-500/[0.04] border border-rose-500/20 text-left space-y-2 text-xs">
          <div className="flex items-center gap-1.5 font-bold text-rose-600 dark:text-rose-400">
            ...
            <span>Đã tự động gom {summary.total - summary.correctCount} câu sai vào Ngân Hàng Lỗi Sai</span>
          </div>
          <p className="text-muted-foreground leading-relaxed text-[11px]">
            Hệ thống đã phân loại lỗi sai theo bẫy khảo thí tương ứng (Bẫy lượng từ, So sánh hơn giả định hoặc Suy diễn ngoài bài) để bạn dễ dàng luyện tập triệt tiêu lỗi ở Module 5.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 pt-2">
        ...
        >
          ...
          <span>Luyện lại bài đọc này</span>
        </button>
        ...
        >
          <span>Ôn tập bẫy trong Error Bank</span>
