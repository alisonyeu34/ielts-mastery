...
import { ProductiveEvaluationResult } from "@/lib/productiveGateChecker";
import {
  HelpCircle,
  CheckCircle2,
  XCircle,
...
export function ProductiveGatewayQuiz({
  questions,
  userAnswers,
  onSelectOption,
  isSubmitted,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-purple-500/10 text-purple-600 dark:text-purple-400">
              CỔNG KIỂM TRA CHIẾN LƯỢC SẢN XUẤT (PRODUCTIVE MASTERY CHECK)
            </span>
            <h2 className="text-lg sm:text-xl font-black text-foreground mt-0.5">
              Đạt Tối Thiểu 80% Để Mở Khóa Phòng Luyện Tập Thực Tế
            </h2>
          </div>
        </div>
      ...
      {isSubmitted && evaluation && (
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 font-bold font-mono text-sm">
              {isPassed ? (
                <>
                  ...
                  <span>XUẤT SẮC! BẠN ĐÃ LÀM CHỦ CHIẾN LƯỢC SẢN XUẤT (≥80%)</span>
                </>
              ) : (
                <>
                  <AlertTriangle className="h-5 w-5 text-rose-500" />
                  <span>CHƯA ĐẠT 80% • CÂU SAI ĐÃ ĐƯỢC LƯU VÀO SỔ LỖI SAI (ERROR BANK)</span>
                </>
              )}
            </div>
            ...
            {isPassed ? (
              <button
                ...
                className="px-4 py-2 rounded-xl bg-emerald-600 text-white font-bold hover:bg-emerald-700 shadow-sm cursor-pointer"
              >
                Vào Phòng Luyện ➔
              </button>
            ) : (
              <button
                type="button"
                onClick={onRetake}
                className="px-4 py-2 rounded-xl bg-rose-600 text-white font-bold hover:bg-rose-700 shadow-sm flex items-center gap-1.5 cursor-pointer"
              >
          ...
          <p className="leading-relaxed">
            {isPassed
              ? "Bạn đã hoàn toàn làm chủ tư duy giám khảo và tiêu chí chấm điểm. Phòng luyện tập thực hành đã được mở khóa thành công!"
              : "Hệ thống đã tự động lưu các câu làm sai vào Error Bank. Hãy đọc lại phân tích của giám khảo bên dưới và bấm 'Làm Lại Quiz' để vượt cổng nhé."}
          </p>
        </div>
      )}
              ...
              {isSubmitted && (
                <div className="p-3.5 rounded-2xl bg-secondary/40 border border-border/80 text-[11px] leading-relaxed pl-8 space-y-1">
                  <span className="font-bold text-foreground font-mono uppercase text-[10px] block">
                    💡 Lý giải của Giám khảo Cambridge:
                  </span>
                  <p className="text-muted-foreground">{q.examinerReasoning}</p>
                </div>
              )}
            </div>
          );
