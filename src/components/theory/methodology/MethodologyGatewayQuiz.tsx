...
import { MethodologyEvaluationResult } from "@/lib/methodologyGateChecker";
import {
  HelpCircle,
  CheckCircle2,
  XCircle,
...
export function MethodologyGatewayQuiz({
  questions,
  userAnswers,
  onSelectOption,
  isSubmitted,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-[10px] font-mono font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-purple-500/10 text-purple-600 dark:text-purple-400">
              CỔNG KIỂM TRA PHƯƠNG PHÁP LUẬN (GATEWAY MASTERY CHECK)
            </span>
            <h2 className="text-lg sm:text-xl font-black text-foreground mt-0.5">
              Đạt Tối Thiểu 80% Để Mở Khóa Phòng Luyện Tập Thực Hành
            </h2>
          </div>
        </div>
      ...
      {isSubmitted && evaluation && (
        ...
        >
            ...
            <div className="flex items-center gap-2 font-bold font-mono text-sm">
              {isPassed ? (
                <>
                  ...
                  <span>XUẤT SẮC! BẠN ĐÃ LÀM CHỦ PHƯƠNG PHÁP KHẢO THÍ (≥80%)</span>
                </>
              ) : (
                <>
          ...
          <p className="leading-relaxed">
            {isPassed
              ? "Bạn đã hoàn toàn thấu hiểu bản chất dạng bài và cách hóa giải các bẫy tinh vi của Cambridge. Phòng luyện tập thực hành đã sẵn sàng!"
              : "Hệ thống đã tự động lưu các câu làm sai vào Error Bank. Hãy xem kỹ phân tích bẫy bên dưới và bấm 'Làm Lại Quiz' để vượt cổng nhé."}
          </p>
        </div>
      )}
              ...
              {isSubmitted && (
                <div className="p-3.5 rounded-2xl bg-secondary/40 border border-border/80 text-[11px] leading-relaxed pl-8 space-y-1">
                  <span className="font-bold text-foreground font-mono uppercase text-[10px] block">
                    💡 Phân tích chiến lược khảo thí:
                  </span>
                  <p className="text-muted-foreground">{q.trapAnalysis}</p>
                </div>
              )}
            </div>
          );
