...
interface StrategyUnlockModalProps {
  isOpen: boolean;
  onClose: () => void;
  title: string;
...
export function StrategyUnlockModal({
  isOpen,
  onClose,
  title,
  scorePercentage,
      ...
      <div className="relative w-full max-w-lg rounded-3xl border border-border bg-card p-6 sm:p-8 shadow-2xl space-y-6 text-center">
        ...
        <div className="space-y-2">
          ...
          <h3 className="text-xl font-black text-foreground">
            Làm Chủ Chiến Lược & Mở Khóa Thực Hành!
          </h3>
          <p className="text-xs text-muted-foreground leading-relaxed max-w-sm mx-auto">
            Bạn đã xuất sắc vượt qua cổng kiểm tra phương pháp của bài <strong>"{title}"</strong>.
          </p>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-indigo-500/[0.04] border border-indigo-500/30 text-left space-y-2 text-xs">
          <div className="flex items-center gap-1.5 text-indigo-600 dark:text-indigo-400 font-bold font-mono text-[10px] uppercase">
            ...
            <span>Phòng Luyện Tập Tương Ứng Đã Mở Khóa:</span>
          </div>

          <h4 className="font-extrabold text-foreground text-sm">
            {unlockedRoute.nameVi}
          </h4>

          <p className="text-muted-foreground leading-relaxed text-[11px]">
            {unlockedRoute.descriptionVi}
          </p>
        </div>
        ...
        <div className="space-y-2.5 pt-2">
          ...
          >
            ...
            <span>Chuyển Sang Phòng Luyện Thực Tế ➔</span>
          </Link>
