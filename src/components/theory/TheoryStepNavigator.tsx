...
interface TheoryStepNavigatorProps {
  currentStep: 1 | 2 | 3 | 4;
...
export function TheoryStepNavigator({
  currentStep,
  onSelectStep,
  isGatePassed,
  className,
}: TheoryStepNavigatorProps) {
  const steps = [
    {
      number: 1 as const,
      title: "Bước 1: Bản Chất & Khái Niệm",
    ...
    {
      number: 3 as const,
      title: "Bước 3: Mổ Xẻ Câu Band 8.5+",
      subtitle: "High-Band Model Dissection",
    ...
    >
      <div className="flex items-center justify-between">
        <span className="text-xs font-extrabold text-foreground uppercase tracking-wider flex items-center gap-1.5">
          ...
          <span>Quy Trình Sư Phạm 3 Bước + Cổng Mở Khóa Thực Hành:</span>
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5">
        {steps.map((s) => {
          ...
          const isDone = isGatePassed || currentStep > s.number;
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive
                  ? `bg-gradient-to-r text-white shadow-md ring-2 ring-red-500/20 ${s.color}`
                  : "bg-card border-border hover:border-red-500/40 hover:bg-secondary/60 text-foreground"
              )}
            >
